      ...
      <el-main class="firstmain">
        <el-container>
          <el-header class="secondheader" style="height: 200px">
            <h1 style="height: 100px;margin-top: 75px;font-size: 20pt;font-family: 黑体">贷款公司申诉信息</h1>
          </el-header>
...
export default {
  created(){
    const _this = this
    axios.get('http://localhost:8081/getAllComplainmessage').then(function (resp){
  ...
  methods:{
    ...
    handleClick(row) {
      ...
      this.$axios.post('http://localhost:8081/deleteComplainmessage',row).then(res =>{
        console.log(res);
